...
class NetworkMonitor(app_manager.RyuApp):
    ...
    def _monitor(self):
        ...
        while 1:
            self.stats['flow'] = {}
            self.stats['port'] = {}
            # for each registered datapath, or switch.
    ...
    def _get_free_bw(self, capacity, speed):
        # BW:Mbit/s
        return max(capacity- speed * 8/10**6, 0)
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        #print(flow.cookie)
        for stat in sorted([flow for flow in body if (flow.priority == 1 and flow.cookie>0)], key=lambda flow: (flow.match.get('in_port'), flow.match.get('ipv4_dst'))):
            ## Need to add extra values to key here!
            #try:
            #print("key!")
            #print(stat.cookie)
            # I added extra values to the key here so that 
            key = (stat.match['in_port'],  stat.match.get('ipv4_dst'), stat.match.get('ipv4_src'), stat.match.get('ip_proto'), stat.cookie,
                stat.instructions[0].actions[0].port) # port num, ip dst, out port.
            #except:
             #   key = (stat.match['in_port'],  stat.match.get('ipv4_dst'),
              #  stat.instructions[0].actions[0].port) # port num, ip dst, out port.

            value = (stat.packet_count, stat.byte_count,
                     stat.duration_sec, stat.duration_nsec) #packet count, byte count, duration in sec and duration nsec
            self._save_stats(self.flow_stats[dpid], key, value, 9)
            ...
            #print(key)
            self._save_stats(self.flow_speed[dpid], key, speed, 9)
    ...
    def port_desc_stats_reply_handler(self, ev):
        ...
        for p in ev.msg.body:
            ...
            speed = bandwidths.SWITCHESPORTS.get(dpid).get(p.port_no)
